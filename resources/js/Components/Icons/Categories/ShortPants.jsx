export default function ShortPantsIcon(props) {
    return (
        <svg
            version="1.1"
            id="Layer_1"
            xmlns="http://www.w3.org/2000/svg"
            xmlnsXlink="http://www.w3.org/1999/xlink"
            viewBox="0 0 512 512"
            xmlSpace="preserve"
            {...props}
        >
            <path
                style={{ fill: "#5A7DBE" }}
                d="M52.967,8.828v52.966h406.069V8.828c0-4.875-3.953-8.828-8.828-8.828H61.794
    C56.919,0,52.967,3.953,52.967,8.828z"
            />
            <rect
                x="211.863"
                style={{ fill: "#4D71B4" }}
                width="17.655"
                height="61.793"
            />
            <path
                style={{ fill: "#6487C8" }}
                d="M449.516,52.966h-69.929V8.828c0-4.875-3.948-8.828-8.828-8.828c-4.879,0-8.828,3.953-8.828,8.828
    v44.138H150.07V8.828c0-4.875-3.948-8.828-8.828-8.828s-8.828,3.953-8.828,8.828v44.138H62.486
    c-10.017,0-18.027,8.324-17.642,18.334l16.623,432.212c0.182,4.74,4.077,8.489,8.821,8.489h150.723c4.748,0,8.646-3.756,8.821-8.501
    l8.21-221.67c0.352-9.49,8.147-17.002,17.643-17.002h0.629c9.496,0,17.291,7.512,17.643,17.002l8.21,221.67
    c0.175,4.745,4.073,8.501,8.821,8.501h150.723c4.744,0,8.639-3.748,8.821-8.489l16.623-432.212
    C467.543,61.29,459.533,52.966,449.516,52.966z"
            />
            <g>
                <path
                    style={{ fill: "#5A7DBE" }}
                    d="M48.139,156.966h31.31v-51.034c0-4.875-3.953-8.828-8.828-8.828H45.837L48.139,156.966z"
                />
                <path
                    style={{ fill: "#5A7DBE" }}
                    d="M221.012,512c4.748,0,8.646-3.756,8.821-8.501l0.993-26.809H60.436l1.032,26.822
    c0.182,4.74,4.077,8.489,8.821,8.489H221.012z"
                />
                <path
                    style={{ fill: "#5A7DBE" }}
                    d="M281.176,476.69l0.993,26.809c0.175,4.745,4.073,8.501,8.821,8.501h150.723
    c4.744,0,8.639-3.748,8.821-8.489l1.032-26.822H281.176z"
                />
            </g>
            <g>
                <circle
                    style={{ fill: "#4D71B4" }}
                    cx="256.001"
                    cy="26.483"
                    r="8.828"
                />
                <path
                    style={{ fill: "#4D71B4" }}
                    d="M459.036,176.552h4.074l1.018-26.483h-5.092c-24.376,0-44.138-19.762-44.138-44.138V52.966h-26.483
    v52.966C388.415,144.934,420.033,176.552,459.036,176.552z"
                />
                <path
                    style={{ fill: "#4D71B4" }}
                    d="M52.967,176.552h-4.074l-1.018-26.483h5.092c24.376,0,44.138-19.762,44.138-44.138V52.966h26.483
    v52.966C123.587,144.934,91.969,176.552,52.967,176.552z"
                />
            </g>
            <path
                style={{ fill: "#5A7DBE" }}
                d="M256.001,52.966h-8.828h-8.828v226.637c1.393-8.399,8.609-14.775,17.341-14.775h0.314V220.69h8.828
    c4.875,0,8.828-3.953,8.828-8.828V52.966H256.001z"
            />
        </svg>
    );
}
